...
const ProcessDetails: React.FC = () => {
  ...
  // ── Process data ──
  const [overallCost, setOverallCost] = useState<CostBreakdown>({ totalRawMaterialCost: 0, totalFinishedGoodsCost: 0, totalScrapCost: 0, totalOtherCharges: 0, grandTotal: 0 });
  ...
  const getStageFromStatus = useCallback((s: ProductionStatus) => {
    switch (s) {
      ...
      case "fg_produced": return "FG Produced";
      case "fg_testing":  return "FG Testing";
  ...
  useEffect(() => {
    ...
    const overall = levels.reduce((acc, l) => {
      ...
      return { totalRawMaterialCost: acc.totalRawMaterialCost + c.totalRawMaterialCost, totalFinishedGoodsCost: acc.totalFinishedGoodsCost + c.totalFinishedGoodsCost, totalScrapCost: acc.totalScrapCost + c.totalScrapCost, totalOtherCharges: acc.totalOtherCharges + c.totalOtherCharges, grandTotal: acc.grandTotal + c.grandTotal };
      ...
      try {
        ...
        if (res?.status && Array.isArray(res.data)) {
          setItems(res.data.map((item: any) => ({
            ...
            regularBuyingPrice: item.regularBuyingPrice || "0", regularSellingPrice: item.regularSellingPrice || "0",
          })));
        }
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        setLoading(true);
        const res = await productionAPI.getProductionById(parseInt(id));
        if (res?.status && res.data) {
          const d = res.data;
          setProcessDetails({
            processNumber: d.docNumber || "", orderDeliveryDate: d.orderDeliveryDate || "",
            expectedCompletionDate: d.expectedCompletionDate || "", status: d.status || "planned",
            ...
            fgStore: d.fgStore ? { name: d.fgStore.name || "", address1: d.fgStore.address1 || "", city: d.fgStore.city || "", postalCode: d.fgStore.postalCode || "", id: d.fgStore.id || 0 } : { name: "", city: "", postalCode: "", id: 0 },
            rmStore: d.rmStore ? { name: d.rmStore.name || "", address1: d.rmStore.address1 || "", city: d.rmStore.city || "", postalCode: d.rmStore.postalCode || "", id: d.rmStore.id || 0 } : { name: "", city: "", postalCode: "", id: 0 },
            scrapStore: d.scrapStore ? { name: d.scrapStore.name || "", address1: d.scrapStore.address1 || "", city: d.scrapStore.city || "", postalCode: d.scrapStore.postalCode || "", id: d.scrapStore.id || 0 } : { name: "", city: "", postalCode: "", id: 0 },
            description: (d as any).description || "", comments: (d as any).comments || "", bomId: d.bom?.id || 0,
          });
          if (d.productionItems && Array.isArray(d.productionItems)) {
            setLevels(d.productionItems.map((item: any) => ({
              expanded: { finishedGoods: true, rawMaterials: true, routing: true, scrap: true, otherCharges: true },
              id: item.id || Date.now(), bomItemId: item.bomItem?.id || 0,
              finishedGoods: (item.finishedGoods || []).map((f: any) => ({ ...f, itemName: f.bomFinishedGoods?.item?.name || "Unknown", itemData: items.find((i) => i.id === f.bomFinishedGoods?.item?.id?.toString()), bomFinishedGoodsId: f.bomFinishedGoods?.id || 0, unit: f.bomFinishedGoods?.unit?.name || "Kg", costAlloc: f.costAlloc || 0, forRepair: f.forRepair || 0, repaired: f.repaired || 0, accept_quantity: f.accept_quantity || f.total_mfg_quantity || 0 })),
              rawMaterials: (item.rawMaterials || []).map((r: any) => ({ ...r, itemName: r.bomRawMaterial?.item?.name || "Unknown", itemData: items.find((i) => i.id === r.bomRawMaterial?.item?.id?.toString()), bomRawMaterialId: r.bomRawMaterial?.id || 0, currentStock: items.find((i) => i.id === r.bomRawMaterial?.item?.id?.toString())?.currentStock || "0", estimatedProduction: r.estimatedProduction || 0, produced: r.produced || 0, unit: r.bomRawMaterial?.unit?.name || "Kg" })),
              routing: (item.routing || []).map((r: any) => ({ routingId: r.bomRouting?.routing?.id, productionRoutingId: r.id, routingNumber: r.bomRouting?.routing?.number, routingName: r.bomRouting?.routing?.name, is_done: r.is_done || false, quantity_completed: r.quantity_completed || 0, comment: r.comment || "", createdAt: r.createdAt || new Date().toISOString() })),
              scrap: (item.scrap || []).map((s: any) => ({ ...s, itemName: s.bomScrap?.item?.name || "Unknown", itemData: items.find((i) => i.id === s.bomScrap?.item?.id?.toString()), bomScrapId: s.bomScrap?.id || 0, unit: s.bomScrap?.unit?.name || "Kg" })),
              otherCharges: (item.other_charges || []).map((oc: any) => ({ ...oc, bomOtherChargeId: oc.bomOtherCharge?.id || 0, bomOtherCharge: oc.bomOtherCharge })),
            })));
          } else {
            setLevels([{ expanded: { finishedGoods: true, rawMaterials: true, routing: true, scrap: true, otherCharges: true }, id: 1, bomItemId: 0, finishedGoods: [], rawMaterials: [], routing: [], scrap: [], otherCharges: [] }]);
          }
        }
      } catch (e) { console.error(e); toast.error("Failed to load process details"); }
      finally { setLoading(false); }
    };
  ...
  // ── TakeActions save → build approval rows → open approval dialog ──
  const handleTakeActionsSave = useCallback(async (saveData: {
    ...
    // ── 1. Build fg_data — keyed by production FG record id, only changed rows ──
    const fg_data: TakeActionPayload["form_data"]["fg_data"] = {};
    for (const row of saveData.fgRows) {
      if ((row.quantityAdded ?? 0) > 0) {
        fg_data[row.itemId] = {
          change_quantity: Number(row.quantityAdded),
          comment: row.comment || "",
        };
      }
    }

    // ── 2. Build rm_data — keyed by production RM record id ──
    const rm_data: TakeActionPayload["form_data"]["rm_data"] = {};
    for (const row of saveData.rmRows) {
      if ((row.quantity ?? 0) > 0) {
        // Map dialog "addLess" label → API change_type
    ...
    // ── 3. Build scrap_data — keyed by production Scrap record id ──
    const scrap_data: TakeActionPayload["form_data"]["scrap_data"] = {};
    for (const row of saveData.scrapRows) {
      // actualQty is the entered value (can differ from estimatedQty)
      const qty = Number(row.actualQty ?? 0);
      if (qty > 0) {
        scrap_data[row.itemId] = {
          change_quantity: qty,
          comment: row.comment || "",
        };
      }
    ...
    try {
      ...
      // ── Normalise: handle both { status, message } body directly
      // or axios wrapper { data: { status, message } } ──
      ...
      try {
        const refresh = await productionAPI.getProductionById(parseInt(processId));
        const d = (refresh as any)?.data ?? refresh;
        if (d?.status !== false && d?.productionItems) {
      ...
      // Axios throws on 4xx/5xx — extract the API message from response body
      const apiMsg =
  ...
  }, [processId, takeActionsSection, processDetails.createdBy, processDetails.processNumber, processDetails.rmStore]);

  // ── FG Testing save → POST /production/proccess/{id}/fg-test ──
  const handleFGTestingSave = useCallback(async (data: FGTestingData[]) => {
    ...
    // ── Refresh production data so FG row values update ──
    try {
      const res = await productionAPI.getProductionById(parseInt(processId));
      if (res?.status && res.data) {
        const d = res.data;
        setProcessDetails((prev) => ({
          ...prev,
        ...
        if (d.productionItems && Array.isArray(d.productionItems)) {
          setLevels(d.productionItems.map((item: any) => ({
            ...
            routing: (item.routing || []).map((r: any) => ({
              routingId: r.bomRouting?.routing?.id, productionRoutingId: r.id,
              routingNumber: r.bomRouting?.routing?.number, routingName: r.bomRouting?.routing?.name,
              is_done: r.is_done || false, quantity_completed: r.quantity_completed || 0,
              comment: r.comment || "", createdAt: r.createdAt || new Date().toISOString(),
            })),
            scrap: (item.scrap || []).map((s: any) => ({
              ...s, itemName: s.bomScrap?.item?.name || "Unknown",
              itemData: items.find((i) => i.id === s.bomScrap?.item?.id?.toString()),
              bomScrapId: s.bomScrap?.id || 0, unit: s.bomScrap?.unit?.name || "Kg",
            })),
            otherCharges: (item.other_charges || []).map((oc: any) => ({
              ...oc, bomOtherChargeId: oc.bomOtherCharge?.id || 0, bomOtherCharge: oc.bomOtherCharge,
            })),
          })));
        }
      }
    ...
    setLevels((prev) =>
      prev.map((l, i) => {
        ...
        return {
          ...
          rawMaterials:
            type === "rm"
              ? l.rawMaterials.filter((_, j) => j !== index)
              : l.rawMaterials,
          scrap:
            type === "scrap"
              ? l.scrap.filter((_, j) => j !== index)
              : l.scrap,
        };
      })
    );
          ...
          arr.map((row, j) =>
            j === index
              ? {
                  ...
                  unit:         itemData?.unit?.name   || "Kg",
                  currentStock: (itemData as any)?.currentStock ?? row.currentStock,
                }
              : row
          );
  ...
  const removeRouting = useCallback((levelIndex: number, index: number) => {
    setLevels((prev) =>
      prev.map((l, i) => {
        ...
        return {
          ...l,
          routing: l.routing.filter((_, j) => j !== index),
        };
      })
    );
  }, []);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // ── Map the full ProductionStatus → the subset UpdateProductionRequest accepts ──
      // fg_produced / fg_testing are set by dedicated API calls (take-action / fg-test),
      // so when the user saves while in those states we keep them as "in_progress".
      ...
      const statusToUpdateable: Record<ProductionStatus, UpdateableStatus> = {
        planned:     "planned",
        publish:     "in_progress",   // "publish" → "in_progress" on save
        in_progress: "in_progress",
        fg_produced: "in_progress",   // read-only state; preserve as in_progress for the update call
        fg_testing:  "in_progress",   // read-only state; preserve as in_progress for the update call
        complete:    "complete",
        cancelled:   "cancelled",
      };
      const apiStatus: UpdateableStatus = statusToUpdateable[processDetails.status];

      // ── Build productionItems payload ──
      ...
      if (res.status) {
        ...
        setProcessDetails((p) => ({
          ...p,
          // Keep the original full status in UI — only the API call used the mapped value
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const raw = await productionAPI.publishProduction(parseInt(processId));
      const res = (raw as any)?.data ?? raw;
      if (res?.status !== false) {
  ...
  const handleStartProcess = async () => {
    ...
    try {
      setActionLoading(true);
      const raw = await productionAPI.startProduction(parseInt(processId));
      const res = (raw as any)?.data ?? raw;
      if (res?.status !== false) {
  ...
  /** Build warning list for Complete dialog, matching the reference UI */
    ...
    levels.forEach((l) => {
      l.finishedGoods.forEach((f) => {
        if ((f.total_mfg_quantity || 0) < f.req_quantity) {
          warnings.push(
            `FG Produced: ${f.total_mfg_quantity || 0} ${f.unit || "Kg"} is less than target ${f.req_quantity} ${f.unit || "Kg"}`
          );
        }
      });
      l.rawMaterials.forEach((r) => {
        if ((r.actual_used_quantity || 0) === 0 && (r.req_quantity || 0) > 0) {
          warnings.push(
            `Quantity issued for 1 or more RM is 0`
          );
          return; // only add once per level
        }
      });
    });
  ...
  const handleComplete = async () => {
    ...
    try {
      setActionLoading(true);
      const raw = await productionAPI.completeProduction(parseInt(processId));
      const res = (raw as any)?.data ?? raw;
      if (res?.status !== false) {
  ...
  const handleCancel = async () => {
    ...
    try {
      setActionLoading(true);
      const raw = await productionAPI.cancelProduction(parseInt(processId));
      const res = (raw as any)?.data ?? raw;
      if (res?.status !== false) {
  ...
  // All FG items from all levels (for FGTestingSidebar)
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                    ...
                    processDetails.status === "fg_produced" ? "bg-orange-100 text-orange-800" :
                    processDetails.status === "fg_testing"  ? "bg-purple-100 text-purple-800" :
      ...
      <div className="sticky bottom-0 border-t bg-white shadow-lg">
        <div className="max-w-8xl mx-auto px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            {/* ── PENDING (publish): only Start Process ── */}
            {!isEditMode && processDetails.status === "publish" && (
              <Button
                onClick={() => setStartProcessConfirmOpen(true)}
                className="bg-[#105076] hover:bg-[#0d4566]"
                disabled={actionLoading}
              >
            ...
            {/* ── WIP (in_progress): Take Actions + Complete Process ── */}
            {!isEditMode && processDetails.status === "in_progress" && (
              <>
                <Button
                  onClick={() => openTakeActions("markFGProduced")}
                  className="bg-[#105076] hover:bg-[#0d4566]"
                  disabled={actionLoading}
                >
            ...
            {/* ── Cancel (all non-terminal states) ── */}
      ...
      <Dialog open={publishConfirmOpen} onOpenChange={setPublishConfirmOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Are you sure you want to publish the document?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-gray-600 mt-2">
            By Publishing the process, Master Process and all Child Processes will be published.
          </p>
      ...
      <Dialog open={startProcessConfirmOpen} onOpenChange={setStartProcessConfirmOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Are you sure you want to Start process?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-gray-600 mt-2">
            This will change the Status of all related process to WIP
          </p>
          <div className="flex justify-end gap-3 mt-6">
      ...
      <Dialog open={completeConfirmOpen} onOpenChange={setCompleteConfirmOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Are you sure you want to complete the Production process?</DialogTitle>
          </DialogHeader>
          ...
          <p className="text-sm text-gray-600 mt-3">
            Note: All sub-processes will be marked as complete.
          </p>
          {levels.flatMap((l) => l.finishedGoods).map((fg) => (
            <p key={fg.id} className="text-xs text-gray-500">
              Total FG Produced {fg.itemName}: {fg.total_mfg_quantity || 0} {fg.unit || "Kg"}
            </p>
          ))}
          <div className="flex justify-end gap-3 mt-6">
      ...
      {/* ── StoreIssueApprovalDialog — opens automatically after TakeActions save ── */}
      <StoreIssueApprovalDialog
        open={approvalOpen}
